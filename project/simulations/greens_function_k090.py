"""Green's function simulation at k_eff ≈ 0.90 (R = 24.500 cm).

Mirrors project/simulations/greens_function.py with the calibrated radius from
KEFF_CONFIGS[0.90]. Run with: python -m project.simulations.greens_function_k090
"""
import numpy as np
import openmc
...
CONFIG = KEFF_CONFIGS[0.90]
RADIUS = CONFIG["radius"]
NAME = f"greens-function-{CONFIG['label']}"

materials = openmc.Materials([mixture_material, detector_material])
geometry = core_with_detector(
    core_center=(0.0, 0.0, 0.0),
    core_radius=RADIUS,
    detector_center=(0.0, 0.0, RADIUS + 10.0 + 5.0),  # along +z, aligned with mesh polar axis
    detector_radius=5.0,
)
...
settings["source"] = openmc.IndependentSource(
    space=openmc.stats.SphericalIndependent(
        r=openmc.stats.PowerLaw(0, RADIUS, 2),  # p(r) ∝ r² → uniform volume
